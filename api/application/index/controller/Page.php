...
class Page extends Auth {
    ...
    public function insert() {
        ...
        if ($this->GetAuth($path)) {
			...
			$array['Zan'] = $request->post('answer.Zan');
			$array['Ding'] = $request->post('answer.Ding');
			$array['Cai'] = $request->post('answer.Cai');
            ...
            if (!$validate->scene('Update')->check($array)) {
                ...
                for ($i = 0;$i < count($data['Reply']);$i++) {
                    if ($data['Reply'][$i]['ID'] == $array['ID']) {
						...
						$data['Reply'][$i]['Zan'] = $array['Zan'];
						$data['Reply'][$i]['Ding'] = $array['Ding'];
						$data['Reply'][$i]['Cai'] = $array['Cai'];
